...
Polygon* CreatePolygon(const std::string &name, const std::vector<Point> &points) {
  ...
  try{
    ...
    try{
      answer = new IsoscelesObtuseTriangle(name,points);
    } 
    catch (int) { 
      try{
        answer = new ObtuseTriangle(name,points);
      } 
      catch (int) { 
        try {
...
# define InitializeCount(type) std::vector<std::string> all_##type
  ...
  std::vector<std::string> obtuse_angle;
  std::vector<std::string> acute_angle;
...
#   define IncrementCount(type) if (dynamic_cast<type*> (*i)) all_##type.push_back((*i)->getName())
    ...
    IncrementCount(ObtuseTriangle);
    IncrementCount(IsoscelesObtuseTriangle);
    IncrementCount(EquilateralTriangle);
    IncrementCount(Quadrilateral);
    IncrementCount(Trapezoid);
    IncrementCount(Kite);
    IncrementCount(Arrow);
    ...
    if ((*i)->HasAnObtuseAngle()) obtuse_angle.push_back((*i)->getName());
    if ((*i)->HasAnAcuteAngle()) acute_angle.push_back((*i)->getName());
    if ((*i)->IsConvex()) convex.push_back((*i)->getName());
    if ((*i)->IsConcave()) concave.push_back((*i)->getName());
  }    
...
# define PrintCount(type) do { ostr << all_##type.size() << " " #type"(s): "; PrintVector(all_##type); } while (0)
  ...
  PrintCount(ObtuseTriangle);
  PrintCount(IsoscelesObtuseTriangle);
  ...
  ostr << obtuse_angle.size() << " Shape(s) with an obtuse angle: ";
  PrintVector(obtuse_angle);
  ostr << acute_angle.size() << " Shape(s) with an acute angle: ";
  PrintVector(acute_angle);
  ostr << convex.size() << " Convex Shape(s): ";
  PrintVector(convex);
  ostr << concave.size() << " Concave Shape(s): ";
  PrintVector(concave);
}
