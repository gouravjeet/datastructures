...
main()
{
  ...
  int month, day, year=2014;
  int montharray[10];
  int daysarray[10];
  int julianarray[10];
  for(int i=0;i<9;i++)
  {
    std::cin >> montharray[i] >> daysarray[i];
    std::cout << julian_day( montharray[i], daysarray[i], year ) << std::endl;
    std::cout << "That is Julian day " << julian_day( montharray[i], daysarray[i], year ) << std::endl;
  }
  return 0;
}