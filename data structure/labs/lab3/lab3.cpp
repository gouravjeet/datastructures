...
void label_stack(uintptr_t* address, const std::string& tag) {
 ...
 void change_times( Time & t1, Time t2){
      ...
      t2.setHour(t2.getHour()+4);
      label_stack((uintptr_t*)&t1,"t1 after edit");
      label_stack((uintptr_t*)&t2,"t2 after edit");
      //print_stack();
    }
...
void compute_squares( uintptr_t c[], uintptr_t b[], uintptr_t n){
...
int main() {
  ...
  //label_stack(&b[3],"b[3]");
  label_stack(&b[4],"b[4]");
  //label_stack(uintptr_t * );
  compute_squares(c,b,n);
  

  Time t1(1,2,3);
  Time t2(3,4,5);

  label_stack((uintptr_t*)&t1,"t1 before edit");
  label_stack((uintptr_t*)&t2,"t2 before edit");
  print_stack();
  change_times(t1,t2);
