...
// CheckPoint 2
int count_paths(int x, int y,vector <vector<bool> > const& blocked){
  if(x==0 && y==0)
    return 1;
  else if(x<0 || y<0 || blocked[x][y])
    return 0;
  else
    return count_paths(x-1,y,blocked)+ count_paths(x,y-1,blocked);
}
// Checkpoint 3
list<Point> find_path(int x, int y, vector<vector<bool> > const& blocked){
  Point this_point(x,y);
  // If we have made it to the origin, then create and return a list containing (0,0)
  if(x==0 && y==0){
    list<Point> result;
    result.push_front(this_point);
    return result;
  }
  //If we have fallen out of the 1st quadtant or reached a blocked position , return an empty list
  else if(x< 0 || y < 0 || blocked[x][y])
  {
    list<Point> empty_list;
    return empty_list;
  }
  // Otherwise recursively search and then choose a non-empty path to use
  else {
    list<Point> over_result=find_path(x-1,y,blocked);
    cout<<"@"<<x<<"("<<endl;
    cout<<"#"<<y<<"("<<endl;
    list<Point> down_result=find_path(x,y-1,blocked);
    cout<<"$"<<x<<")"<<endl;
    cout<<"&"<<y<<")"<<endl;
    if(over_result.empty() && down_result.empty()){
      return over_result;
    }
    // if we can't get there using the path over, then use the down path
    else if(over_result.empty())
    {
      down_result.push_front(this_point);
      return down_result;
    }
    // Use the over path
    else{
      over_result.push_front(this_point);
      return over_result;
    }
...
int main(int argc, char* argv[]) {
  ...
  cout<<count_paths(start_x,start_y,blocked_grid)<<endl;
  list<Point> path= find_path(start_x,start_y,blocked_grid);
  if(path.empty())
    cout<<"No path exists";
  else{
    cout<<"Here is the sequence of x, y locations on one of these paths";
    for(list<Point>::iterator p=path.begin();p!=path.end();++p){
      cout<<"("<<p->x<<","<<p->y<<")\n";
    }
