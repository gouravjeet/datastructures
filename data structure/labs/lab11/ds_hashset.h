...
public:
  ...
  iterator begin() 
  {
    ...
    {
      hash_list_itr p = m_table[i].begin();
      for (; p != m_table[i].end(); ++p)
      {
       if(*p=="")
       {
        continue;
...
private:
  ...
  {
    std::vector< std::list<KeyType> > m_table_new(new_size);
    iterator p=this->begin();
    int new_m_size=0;
    for ( ; p != this->end(); ++p )
    {
      unsigned int hash_value = m_hash(*p);
      unsigned int index = hash_value % m_table_new.size();
      m_table_new[index].push_back(*p);
      new_m_size++;
    }
    m_table=m_table_new;
    m_size=new_m_size;
